<template>
  <div class="sim-project-property-list">
    <!-- 搜索筛选区 -->
    ...
    <el-card shadow="never" style="margin-top: 12px">
      ...
      >
        ...
        <el-table-column prop="apply_project" label="适用项目" width="120" />
        ...
        <el-table-column prop="project_property" label="项目资产" width="100">
          <template #default="{ row }">
            <a v-if="row.project_property" :href="row.project_property" target="_blank" class="download-link">
              下载
            </a>
            <span v-else>-</span>
          </template>
...
import {
  getSimProjectPropertyList,
  getSimProjectPropertyCreators,
  deleteSimProjectProperty,
  batchDeleteSimProjectProperties,
} from '@/api/data_manage'
import { usePagination } from '@/composables/usePagination'
import { usePermission } from '@/composables/usePermission'
import SimProjectPropertyFormDialog from './SimProjectPropertyFormDialog.vue'

// ── 权限控制 ────────────────────────────────────────────────
const { hasPermission } = usePermission()
const canView   = hasPermission('data_manage.view_simprojectproperty')
const canAdd    = hasPermission('data_manage.add_simprojectproperty')
const canChange = hasPermission('data_manage.change_simprojectproperty')
const canDelete = hasPermission('data_manage.delete_simprojectproperty')
...
const {
  ...
  handleSizeChange,
  resetAndFetch,
} = usePagination(getSimProjectPropertyList, {
  apply_project: '',
  property_tag: '',
  created_by: null,
...
async function loadCreators() {
  try {
    const res = await getSimProjectPropertyCreators()
...
async function handleBatchDelete() {
  ...
  await batchDeleteSimProjectProperties(selectedIds.value)
...
async function handleDelete(id) {
  await deleteSimProjectProperty(id)
  ElMessage.success('删除成功')
  fetchData()
}
