<template>
  <div class="case-map-list">
    ...
    <el-card shadow="never" style="margin-top: 12px">
      <div class="toolbar">
        <div class="toolbar-left">
          <el-button v-permission="'sim_test_agv.add_casemap'" type="primary" :icon="Plus"
            @click="openCreateDialog">新建地图</el-button>
          <el-button v-permission="'sim_test_agv.delete_casemap'" type="danger" :icon="Delete"
      ...
      <el-table v-loading="loading" :data="tableData" stripe border style="width:100%;margin-top:12px"
        ...
        <el-table-column prop="district_name" label="分区名称" min-width="180" />
        <el-table-column prop="map_file" label="地图文件" width="90">
          <template #default="{ row }">
            <a v-if="row.map_file" :href="row.map_file" target="_blank" class="download-link">下载</a>
            <span v-else>-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="130" fixed="right">
          <template #default="{ row }">
            <el-button v-permission="'sim_test_agv.change_casemap'" type="primary" link @click="openEditDialog(row)">编辑</el-button>
            <el-popconfirm title="确认删除该地图？" @confirm="handleDelete(row.id)">
              <template #reference>
                <el-button v-permission="'sim_test_agv.delete_casemap'" type="danger" link>删除</el-button>
              </template>
            </el-popconfirm>
          </template>
        </el-table-column>
    ...
    <CaseMapFormDialog v-model:visible="formDialogVisible" :edit-data="editRow" @success="fetchData()" />
  </div>
</template>
...
import { getCaseMapList, deleteCaseMap, batchDeleteCaseMaps, getCaseMapCreators } from '@/api/sim_test_agv'
import { usePagination } from '@/composables/usePagination'
import CaseMapFormDialog from './CaseMapFormDialog.vue'

const { loading, tableData, pagination, filters, fetchData, handlePageChange, handleSizeChange, resetAndFetch } =
  usePagination(getCaseMapList)
...
const creators = ref([])
onMounted(async () => {
  const res = await getCaseMapCreators()
...
async function handleBatchDelete() {
  ...
  await batchDeleteCaseMaps(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}
