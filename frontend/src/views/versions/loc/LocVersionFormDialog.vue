...
async function loadEnvOptions() {
  try {
    const res = await locApi.getEnvList({ page_size: 999 })
...
async function handleSubmit() {
  ...
  try {
    if (!isEdit.value) {
      ...
      await locApi.createVersion(fd, (p) => versionFileRef.value?.setProgress(p))
      ElMessage.success('版本创建成功')
    } else {
      await locApi.updateVersion(props.editData.id, {
