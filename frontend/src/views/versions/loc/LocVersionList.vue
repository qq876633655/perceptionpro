<template>
  <div class="loc-version-list">
    ...
    <el-card shadow="never" style="margin-top: 12px">
      <div class="toolbar">
        <div class="toolbar-left">
          ...
          <el-button @click="$router.push('/envs/loc')">版本环境</el-button>
        </div>
    ...
    <LocVersionFormDialog v-model:visible="formDialogVisible" :edit-data="editRow" @success="fetchData()" />
...
import LocVersionFormDialog from './LocVersionFormDialog.vue'

const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_locversion')
const canAdd    = hasPermission('version_pack.add_locversion')
const canChange = hasPermission('version_pack.change_locversion')
const canDelete = hasPermission('version_pack.delete_locversion')

const {
  loading, tableData, pagination, filters,
  fetchData, handlePageChange, handleSizeChange, resetAndFetch,
} = usePagination(locApi.getVersionList)
...
onMounted(async () => {
  ...
  try {
    const [envRes, creatorsRes] = await Promise.all([
      locApi.getEnvList({ page_size: 999 }),
      locApi.getVersionCreators(),
    ])
...
async function handleBatchDelete() {
  ...
  await locApi.batchDeleteVersions(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}

async function handleDelete(id) {
  await locApi.deleteVersion(id)
  ElMessage.success('删除成功')
  fetchData()
}
...
async function cycleTestResult(row) {
  ...
  try {
    await locApi.updateVersion(row.id, { test_result: next })
...
<style scoped>
.loc-version-list { max-width: 1400px; }
