<template>
  <div class="at-env-list">
    ...
    <AtEnvFormDialog v-model:visible="formDialogVisible" :edit-data="editRow" @success="fetchData()" />
...
import AtEnvFormDialog from './AtEnvFormDialog.vue'

const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_atenv')
const canAdd    = hasPermission('version_pack.add_atenv')
const canChange = hasPermission('version_pack.change_atenv')
const canDelete = hasPermission('version_pack.delete_atenv')

const {
  loading, tableData, pagination, filters,
  fetchData, handlePageChange, handleSizeChange, resetAndFetch,
} = usePagination(atApi.getEnvList)
...
onMounted(async () => {
  if (!canView) return
  try {
    const res = await atApi.getEnvCreators()
...
async function handleBatchDelete() {
  ...
  await atApi.batchDeleteEnvs(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}

async function handleDelete(id) {
  await atApi.deleteEnv(id)
  ElMessage.success('删除成功')
  fetchData()
}
