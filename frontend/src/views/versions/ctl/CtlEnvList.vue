<template>
  <div class="ctl-env-list">
    ...
    <CtlEnvFormDialog v-model:visible="formDialogVisible" :edit-data="editRow" @success="fetchData()" />
...
import CtlEnvFormDialog from './CtlEnvFormDialog.vue'

const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_ctlenv')
const canAdd    = hasPermission('version_pack.add_ctlenv')
const canChange = hasPermission('version_pack.change_ctlenv')
const canDelete = hasPermission('version_pack.delete_ctlenv')

const {
  loading, tableData, pagination, filters,
  fetchData, handlePageChange, handleSizeChange, resetAndFetch,
} = usePagination(ctlApi.getEnvList)
...
onMounted(async () => {
  if (!canView) return
  try {
    const res = await ctlApi.getEnvCreators()
...
async function handleBatchDelete() {
  ...
  await ctlApi.batchDeleteEnvs(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}

async function handleDelete(id) {
  await ctlApi.deleteEnv(id)
  ElMessage.success('删除成功')
  fetchData()
}
