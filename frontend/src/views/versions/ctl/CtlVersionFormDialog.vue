...
async function loadEnvOptions() {
  try {
    const res = await ctlApi.getEnvList({ page_size: 999 })
...
async function handleSubmit() {
  ...
  try {
    if (!isEdit.value) {
      ...
      await ctlApi.createVersion(fd, (p) => versionFileRef.value?.setProgress(p))
      ElMessage.success('版本创建成功')
    } else {
      await ctlApi.updateVersion(props.editData.id, {
