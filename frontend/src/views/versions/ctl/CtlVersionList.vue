<template>
  <div class="ctl-version-list">
    ...
    <el-card shadow="never" style="margin-top: 12px">
      <div class="toolbar">
        <div class="toolbar-left">
          ...
          <el-button @click="$router.push('/envs/ctl')">版本环境</el-button>
        </div>
    ...
    <CtlVersionFormDialog v-model:visible="formDialogVisible" :edit-data="editRow" @success="fetchData()" />
...
import CtlVersionFormDialog from './CtlVersionFormDialog.vue'

const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_ctlversion')
const canAdd    = hasPermission('version_pack.add_ctlversion')
const canChange = hasPermission('version_pack.change_ctlversion')
const canDelete = hasPermission('version_pack.delete_ctlversion')

const {
  loading, tableData, pagination, filters,
  fetchData, handlePageChange, handleSizeChange, resetAndFetch,
} = usePagination(ctlApi.getVersionList)
...
onMounted(async () => {
  ...
  try {
    const [envRes, creatorsRes] = await Promise.all([
      ctlApi.getEnvList({ page_size: 999 }),
      ctlApi.getVersionCreators(),
    ])
...
async function handleBatchDelete() {
  ...
  await ctlApi.batchDeleteVersions(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}

async function handleDelete(id) {
  await ctlApi.deleteVersion(id)
  ElMessage.success('删除成功')
  fetchData()
}
...
async function cycleTestResult(row) {
  ...
  try {
    await ctlApi.updateVersion(row.id, { test_result: next })
...
<style scoped>
.ctl-version-list { max-width: 1400px; }
