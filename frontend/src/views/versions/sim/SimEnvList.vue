<template>
  <div class="sim-env-list">
    ...
    <SimEnvFormDialog v-model:visible="formDialogVisible" :edit-data="editRow" @success="fetchData()" />
...
import SimEnvFormDialog from './SimEnvFormDialog.vue'

const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_simenv')
const canAdd    = hasPermission('version_pack.add_simenv')
const canChange = hasPermission('version_pack.change_simenv')
const canDelete = hasPermission('version_pack.delete_simenv')

const {
  loading, tableData, pagination, filters,
  fetchData, handlePageChange, handleSizeChange, resetAndFetch,
} = usePagination(simApi.getEnvList)
...
onMounted(async () => {
  if (!canView) return
  try {
    const res = await simApi.getEnvCreators()
...
async function handleBatchDelete() {
  ...
  await simApi.batchDeleteEnvs(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}

async function handleDelete(id) {
  await simApi.deleteEnv(id)
  ElMessage.success('删除成功')
  fetchData()
}
