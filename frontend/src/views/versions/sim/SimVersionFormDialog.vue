...
async function loadEnvOptions() {
  try {
    const res = await simApi.getEnvList({ page_size: 999 })
...
async function handleSubmit() {
  ...
  try {
    if (!isEdit.value) {
      ...
      await simApi.createVersion(fd, (p) => versionFileRef.value?.setProgress(p))
      ElMessage.success('版本创建成功')
    } else {
      await simApi.updateVersion(props.editData.id, {
