<template>
  <div class="sim-version-list">
    ...
    <el-card shadow="never" style="margin-top: 12px">
      <div class="toolbar">
        <div class="toolbar-left">
          ...
          <el-button @click="$router.push('/envs/sim')">版本环境</el-button>
        </div>
    ...
    <SimVersionFormDialog v-model:visible="formDialogVisible" :edit-data="editRow" @success="fetchData()" />
...
import { simApi } from '@/api/version'
import { usePagination } from '@/composables/usePagination'
import { usePermission } from '@/composables/usePermission'
import SimVersionFormDialog from './SimVersionFormDialog.vue'

const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_simversion')
const canAdd    = hasPermission('version_pack.add_simversion')
const canChange = hasPermission('version_pack.change_simversion')
const canDelete = hasPermission('version_pack.delete_simversion')

const {
  loading, tableData, pagination, filters,
  fetchData, handlePageChange, handleSizeChange, resetAndFetch,
} = usePagination(simApi.getVersionList)
...
onMounted(async () => {
  ...
  try {
    const [envRes, creatorsRes] = await Promise.all([
      simApi.getEnvList({ page_size: 999 }),
      simApi.getVersionCreators(),
    ])
...
async function handleBatchDelete() {
  ...
  await simApi.batchDeleteVersions(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}

async function handleDelete(id) {
  await simApi.deleteVersion(id)
  ElMessage.success('删除成功')
  fetchData()
}
...
async function cycleTestResult(row) {
  ...
  try {
    await simApi.updateVersion(row.id, { test_result: next })
...
<style scoped>
.sim-version-list { max-width: 1400px; }
