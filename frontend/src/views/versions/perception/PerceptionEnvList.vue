      ...
      >
        ...
        <el-table-column prop="env_note" label="环境描述" width="100">
          <template #default="{ row }">
            <el-button v-if="row.env_note" type="primary" link @click="showDetail(row.env_note, '环境描述')">
              展示
            </el-button>
            <span v-else>-</span>
          </template>
...
import { getEnvList, deleteEnv, batchDeleteEnvs, getEnvCreators } from '@/api/version'
import { usePagination } from '@/composables/usePagination'
import { usePermission } from '@/composables/usePermission'
import PerceptionEnvFormDialog from './PerceptionEnvFormDialog.vue'

// ── 权限控制 ────────────────────────────────────────────────
const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_perenv')
const canAdd    = hasPermission('version_pack.add_perenv')
const canChange = hasPermission('version_pack.change_perenv')
const canDelete = hasPermission('version_pack.delete_perenv')
...
onMounted(async () => {
  if (!canView) return
  try {
    const res = await getEnvCreators()
...
async function handleBatchDelete() {
  await ElMessageBox.confirm(
    `确认删除选中的 ${selectedIds.value.length} 个环境？`,
    '批量删除',
    { type: 'warning' },
  )
  await batchDeleteEnvs(selectedIds.value)
