    ...
    >
      <el-form-item label="环境名称" prop="env_name" :error="serverErrors.env_name">
        <el-input
          ...
          :disabled="isEdit"
          :placeholder="isEdit ? '环境名称创建后不可修改' : '请输入环境名称'"
          clearable
        />
      </el-form-item>
      ...
      <template v-if="isEdit">
        <el-form-item label="当前文件">
          <a
            v-if="props.editData?.env_file"
            :href="props.editData.env_file"
            target="_blank"
            class="download-link"
          >
            {{ extractFilename(props.editData.env_file) }}
          </a>
...
import { createEnv, updateEnv } from '@/api/version'
...
watch(
  ...
  (val) => {
    if (val) {
      ...
      if (props.editData) {
        Object.assign(form, {
          env_name: props.editData.env_name ?? '',
          apply_project: props.editData.apply_project ?? '主线版本',
          env_note: props.editData.env_note ?? '',
          env_file: null,
        })
      } else {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      ...
      formData.append('env_name', form.env_name)
