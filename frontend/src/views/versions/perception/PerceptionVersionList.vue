<template>
  <div class="perception-version-list">
    <!-- 搜索筛选区 -->
    ...
    <el-card shadow="never" style="margin-top: 12px">
      <div class="toolbar">
        <div class="toolbar-left">
          ...
          <el-button
            @click="$router.push('/envs/perception')"
          >
            版本环境
          </el-button>
        </div>
      ...
      >
        ...
        <el-table-column prop="versions_type" label="版本类型" min-width="190">
          <template #default="{ row }">
            <template v-if="Array.isArray(row.versions_type) && row.versions_type.length">
              <el-tag
                v-for="t in row.versions_type"
                :key="t"
                :type="versionTypeTagType(t)"
                size="small"
        ...
        <el-table-column prop="database_file" label="数据库文件" width="90">
          <template #default="{ row }">
            <a v-if="row.database_file" :href="row.database_file" target="_blank" class="download-link">下载</a>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column prop="dev_test_result" label="研发提测" width="90">
          <template #default="{ row }">
            <el-button
              v-if="row.dev_test_result"
              type="primary"
              link
              @click="openDetail('研发提测', row.dev_test_result)"
            >展示</el-button>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column prop="test_result" label="测试结果" width="110">
          <template #default="{ row }">
            <el-button
              :type="testResultTagType(row.test_result)"
              size="small"
              :loading="!!cycling[row.id]"
              :disabled="!canChange"
              @click="cycleTestResult(row)"
            >
              {{ row.test_result ?? '未开始' }}
            </el-button>
          </template>
        ...
        <el-table-column prop="test_verdict" label="测试总结" width="90">
          <template #default="{ row }">
            <el-button
              v-if="row.test_verdict"
              type="primary"
              link
              @click="openDetail('测试总结', row.test_verdict)"
            >展示</el-button>
            <span v-else>-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="140" fixed="right">
          <template #default="{ row }">
            <el-button v-if="canChange" type="primary" link @click="openEditDialog(row)">
              编辑
            </el-button>
            <el-popconfirm
              v-if="canDelete"
...
import { getVersionList, deleteVersion, updateVersion, batchDeleteVersions, getEnvList, getVersionCreators } from '@/api/version'
import { usePagination } from '@/composables/usePagination'
import { usePermission } from '@/composables/usePermission'
import PerceptionVersionFormDialog from './PerceptionVersionFormDialog.vue'

// ── 权限控制 ────────────────────────────────────────────────
const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_perversion')
const canAdd    = hasPermission('version_pack.add_perversion')
const canChange = hasPermission('version_pack.change_perversion')
const canDelete = hasPermission('version_pack.delete_perversion')
...
async function cycleTestResult(row) {
  ...
  cycling[row.id] = true
  const current = row.test_result ?? '未开始'
  const idx = TEST_RESULT_ORDER.indexOf(current)
  const next = TEST_RESULT_ORDER[(idx + 1) % TEST_RESULT_ORDER.length]
  try {
    await updateVersion(row.id, { test_result: next })
    row.test_result = next
  } catch {
    // 错误已在拦截器处理
  } finally {
    delete cycling[row.id]
  }
}
...
function versionTypeTagType(val) {
  return VERSION_TYPE_COLOR[val] ?? ''
}
