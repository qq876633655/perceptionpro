<template>
  <div class="sen-env-list">
    ...
    <SenEnvFormDialog v-model:visible="formDialogVisible" :edit-data="editRow" @success="fetchData()" />
...
import SenEnvFormDialog from './SenEnvFormDialog.vue'

const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_senenv')
const canAdd    = hasPermission('version_pack.add_senenv')
const canChange = hasPermission('version_pack.change_senenv')
const canDelete = hasPermission('version_pack.delete_senenv')

const {
  loading, tableData, pagination, filters,
  fetchData, handlePageChange, handleSizeChange, resetAndFetch,
} = usePagination(senApi.getEnvList)
...
onMounted(async () => {
  if (!canView) return
  try {
    const res = await senApi.getEnvCreators()
...
async function handleBatchDelete() {
  ...
  await senApi.batchDeleteEnvs(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}

async function handleDelete(id) {
  await senApi.deleteEnv(id)
  ElMessage.success('删除成功')
  fetchData()
}
