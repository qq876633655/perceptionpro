<template>
  <div class="sen-version-list">
    ...
    <el-card shadow="never" style="margin-top: 12px">
      <div class="toolbar">
        <div class="toolbar-left">
          ...
          <el-button @click="$router.push('/envs/sen')">版本环境</el-button>
        </div>
    ...
    <SenVersionFormDialog v-model:visible="formDialogVisible" :edit-data="editRow" @success="fetchData()" />
...
import { senApi } from '@/api/version'
import { usePagination } from '@/composables/usePagination'
import { usePermission } from '@/composables/usePermission'
import SenVersionFormDialog from './SenVersionFormDialog.vue'

const { hasPermission } = usePermission()
const canView   = hasPermission('version_pack.view_senversion')
const canAdd    = hasPermission('version_pack.add_senversion')
const canChange = hasPermission('version_pack.change_senversion')
const canDelete = hasPermission('version_pack.delete_senversion')

const {
  loading, tableData, pagination, filters,
  fetchData, handlePageChange, handleSizeChange, resetAndFetch,
} = usePagination(senApi.getVersionList)
...
onMounted(async () => {
  ...
  try {
    const [envRes, creatorsRes] = await Promise.all([
      senApi.getEnvList({ page_size: 999 }),
      senApi.getVersionCreators(),
    ])
...
async function handleBatchDelete() {
  ...
  await senApi.batchDeleteVersions(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}

async function handleDelete(id) {
  await senApi.deleteVersion(id)
  ElMessage.success('删除成功')
  fetchData()
}
...
async function cycleTestResult(row) {
  ...
  try {
    await senApi.updateVersion(row.id, { test_result: next })
...
<style scoped>
.sen-version-list { max-width: 1400px; }
