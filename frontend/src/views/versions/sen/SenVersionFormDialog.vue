...
async function loadEnvOptions() {
  try {
    const res = await senApi.getEnvList({ page_size: 999 })
...
async function handleSubmit() {
  ...
  try {
    if (!isEdit.value) {
      ...
      await senApi.createVersion(fd, (p) => versionFileRef.value?.setProgress(p))
      ElMessage.success('版本创建成功')
    } else {
      await senApi.updateVersion(props.editData.id, {
