...
def _sim_project_property_delete_old_file(sender, instance, **kwargs):
    ...
    try:
        ...
        return
    if old.project_property and old.project_property.name != instance.project_property.name:
...
@receiver(pre_save, sender=SimCommonProperty)
def _sim_common_property_delete_old_file(sender, instance, **kwargs):
    if not instance.pk:
        return
    try:
        old = SimCommonProperty.objects.get(pk=instance.pk)
    except SimCommonProperty.DoesNotExist:
        return
    if old.common_property and old.common_property.name != instance.common_property.name:
        old.common_property.delete(save=False)
